...
For example, the longest proper prefix matching the corresponding suffix of the first three characters of the word 'edified' is 'ed'.
To see this, you must consider the first and last three characters of the string, 'edi' and 'ied'.
Obviously, 'edi' and 'ied'  are not the same string, so we consider the substring prefix and suffix of just two characters 'ed' and 'ed'.
These substrings match, and so the value that would be encoded at the third position in the array (index 2 with a zero-based index) is 2.
