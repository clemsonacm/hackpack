...
For example, the longest proper prefix matching the corresponding suffix of the seven characters of the word 'edified' is 'e'.
To see this, you must consider the first and last three characters of the string, 'ed' and 'ie'.
Obviously, 'ed' and 'ie'  are not the same string, so we consider the substring prefix and suffix of just one character 'e' and 'e'.
These substrings match, and so the value that would be encoded at the seventh position in the array (index 6 with a zero-based index) is 1.
