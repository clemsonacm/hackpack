...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{./algorithms/breadth-first-search/partial-bfs}
	\caption{\small A breadth-first search on a graph with no particular target node.
	The numbers in nodes represents the number of steps the search took to reach that node from the starting node (denoted with a '0').}
\end{figure}
